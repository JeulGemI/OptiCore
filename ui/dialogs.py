...
· 게임 부스터 [v2.1.0 비침습 모드로 개편]: 감시할 게임을 고르고 시작하면,
  - 시작할 때 : 메모리를 딱 1회 정리하고 게임 우선순위를 정상값(Normal)으로
    안정화합니다. 설정에 따라 P-Core 바인딩과 백그라운드 앱 일시 동결도 함께
    적용됩니다. 적용된 항목은 시작 직후 팝업으로 전부 보여줍니다.
  - 게임이 도는 동안 : 살아있는지 5초마다 확인만 합니다. 메모리를 강제로
    회수하거나 우선순위를 다시 건드리는 일이 없습니다.
    (v2.0.x는 이 두 가지를 반복해서 오히려 스터터와 소리 끊김을 만들었습니다)
  - 게임이 끝나면 : 적용한 설정을 전부 원복하고 RAM을 정리합니다.
  ※ 동작 방식은 [⚙️ 설정] 탭의 '게임 부스터 동작 방식'에서 바꿀 수 있습니다.
...
· 게임 작업 프로필 주입 (Tasks\\Games) [v2.1.0 신규]: 멀티미디어 스케줄러
  (MMCSS)가 게임 스레드에 적용하는 규칙을 조정합니다. GPU 우선순위 8,
  스케줄링 범주 High 등을 넣어줍니다. 게임이 스스로 'Games' 범주로 등록한
  스레드에만 적용되므로 다른 프로그램에는 영향이 없습니다.
· Nagle 알고리즘 해제 [v2.1.0 신규]: 작은 패킷을 모아 보내는 기능을 꺼서
  조작 입력의 대기 시간을 줄입니다. 되돌릴 때는 값을 0으로 덮는 게 아니라
  완전히 삭제해 Windows 순정 상태로 정확히 돌아갑니다.
· BCD 타이머 설정 [v2.1.0 신규]: 타이머 주기를 일정하게 만들어 프레임
  페이싱의 흔들림을 줄입니다. 재부팅 후 적용되며, 드물게 특정 메인보드에서
  불안정할 수 있으니 이상하면 순정 복원으로 되돌리세요.
...
※ 여기 있는 항목은 모두 Microsoft가 문서화한 레지스트리 값과 bcdedit
  옵션입니다. 드라이버 후킹이나 커널 조작이 전혀 없어 안티치트가 오탐할
  여지가 없고, [🩺 진단 & 복원]의 '원클릭 순정 복원'으로 전부 되돌릴 수
  있습니다.
...
· 창 닫기(X) 동작 [v2.1.0 신규]: X 버튼을 눌렀을 때의 동작을 고릅니다.
  - 작업 표시줄로 최소화 (기본값): 창은 사라지지만 작업 표시줄 아이콘이
    남아 한 번의 클릭으로 되돌아올 수 있습니다.
  - 시스템 트레이로 숨김: 작업 표시줄에서도 사라지고 우측 하단 트레이에만
    남습니다 (v2.0.x의 동작).
  - 프로그램 완전 종료: X를 누르면 바로 종료합니다.
  ※ 어떤 설정이든 트레이 아이콘 우클릭 → [완전 종료]로는 항상 끌 수 있습니다.
· 게임 부스터 동작 방식 [v2.1.0 신규]:
  - 감시 폴링 주기: 게임이 실행 중인지 확인하는 간격입니다. CPU를 자주 깨우지
    않도록 5초 미만으로는 설정할 수 없습니다.
  - 시작 전 1회 정리 / 우선순위 안정화: 기본으로 켜져 있습니다.
  - Above Normal 허용: 게임 우선순위를 한 단계 올립니다. High나 Realtime으로는
    어떤 경우에도 올라가지 않습니다 (소리 끊김의 원인이라 코드 경로가 없습니다).
  - P-Core 바인딩: 12세대 인텔 이후 하이브리드 CPU에서만 동작합니다. 게임이
    효율 코어(E-Core)로 밀리는 것을 막습니다. 동종 코어 CPU면 아무 일도 하지
    않으며, 원래 설정을 기억했다가 정확히 되돌립니다.
  - 백그라운드 앱 일시 동결: 브라우저·런처를 강제 종료하지 않고 잠시 멈춥니다.
    열어둔 탭과 로그인 상태가 그대로 보존되며 게임이 끝나면 자동 복구됩니다.
    안티치트·오디오·그래픽 드라이버·보안 프로세스는 절대 대상이 되지 않습니다.
    급하면 [📊 성능 대시보드]의 조치 내역에서 [지금 해동]으로 바로 되살릴 수 있습니다.
